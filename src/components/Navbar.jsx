    ...
    <nav className="w-full absolute top-0 left-0 z-50 bg-transparent lg:pt-5 lg:px-10 pt-2 ">
      <div className="container mx-auto flex items-center justify-between px-4 py-4">
          ...
          {["Jobs", "Companies", "About", "Contact"].map((item, idx) => (
            ...
            >
              <NavLink
                to={`/${item.toLowerCase()}`}
                className={({ isActive }) =>
                  `px-3 py-1 rounded-2xl hover:text-blue-200 ${
                    isActive
                      ? "bg-gray-600/60 text-blue-200 shadow"
                      : ""
                  }`
                }
              >
                  ...
                  <>
                    ...
                    <AnimatePresence>
                      {isActive && (
                        <motion.div
                          layoutId="underline"
                          className="absolute left-0 right-0  bg-blue-400 rounded-full"
        ...
        <div className="hidden md:flex gap-4 items-center ">
          <Link to="/login">
            <button className="px-5 py-1 rounded-lg cursor-pointer border border-white shadow text-orange-400 hover:border-orange-500 hover:text-white transition">
              Login
            </button>
          </Link>
